...
for label, folder, f in all_files:
    ...
    cards_html += (
        f'<div class="til-card" data-category="{label}" data-date="{date}">\n'
...
<div class="til-filter">
  <input id="til-search" type="text" placeholder="🔍 날짜 또는 키워드 검색 (예: 2026-06, SQL)">
...
<p id="til-empty" style="display:none;color:#888;text-align:center">검색 결과가 없어요.</p>
...
.til-filter input { width:100%; padding:0.55rem 0.8rem; border:1px solid #ccc; border-radius:8px; font-size:0.95rem; box-sizing:border-box; }
...
(function() {
  ...
  function filter() {
    ...
    cards.forEach(function(card) {
      ...
      var matchCat = activeCategory === '전체' || cat === activeCategory;
      var matchKey = !keyword || text.includes(keyword);
      var show = matchCat && matchKey;
